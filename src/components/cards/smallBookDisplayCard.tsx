...
type SmallBookDisplayCardProps = {
  book: { book: Book, loanedBook?: LoanedBook, booksLoaned: number }
}
        ...
        <div className="relative w-full lg:w-full">
          ...
          <div className="bottom-0 right-0 mb-2 mr-2 px-2 rounded-lg absolute bg-gray-500 text-gray-100 text-xs font-medium">
            <div className="flex flex-row space-x-1 text-gray-100">
              ...
              <span className="text-xs font-medium tracking-tight">{moment(props.book.book.bookPublishedDate).format("Do MMM YY")}</span>
            </div>
          </div>
        </div>

        <div className="flex flex-col w-full h-[4rem] px-2 py-1 bg-white rounded-b-xl">
          <h2 onClick={() => navigate("/book/" + props.book.book.bookId)} className="text-lg font-bold line-clamp-1 pr-2 cursor-pointer">{props.book.book.bookName}</h2>
          <p className="text-gray-500 text-sm line-clamp-1">
            <span className="font-semibold">Category: </span>
            <span>{props.book.book.bookCategory?.categoryName}</span>
          </p>
        </div>
      </div>
    </div>
